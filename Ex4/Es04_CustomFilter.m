...
G_s=P^r/(s+P)^r
D_s=(s+P)^r;
c=coeffs(D_s,s)         % Thanks to coeff there's no need of a temporary variable (var, defined with the butterworth filter)
...
for j=1:1:r
    K(j)=c(j)/c(j+1);   % Coefficient of Differential Equation given the coefficient of polynomial
...
% Real mass computed from flight values 
...
single_plot_pdf(time,wrench_e(3,:),'Time[s]','$$f_{ez} [N]$$','fz.pdf');
